...
public class RateLimitRedisDao {
    ...
    public long incrCount(TimeUnit time, Calendar calendar, String params) {
        String key = RATE_KEY_PREFIX + StringUtils.substring(time.name(), 0, 1).toLowerCase() + ":" + getCurrentTimeNum(calendar, time) + ":" + params;
        Long result = redis.execute((RedisConnection redisConnection) -> redisConnection.incr(key.getBytes()));
